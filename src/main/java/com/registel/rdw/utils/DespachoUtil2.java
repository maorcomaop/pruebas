...
public class DespachoUtil2 {
    ...
    public DespachoUtil2(Despacho dph) {
        // Obtiene listado de configuraciones de despacho
        lst_dph = DespachoBD.select();
        lst_pd  = new ArrayList<PlanillaDespacho>();
    ...
    public void generarDespacho(Date fecha_inicio, Date fecha_final) {
        ...
        int id_ruta, dia_semana = 0, ndias = 0;
        int nrutas = lst_idRuta.size();        
        ...
        do {                        
            ...
            for (int i = 0; i < nrutas; i++) {
                ...
                if (conf_dph != null) {                                                       
                    if (ndias >= nrutas) {
                        conf_dph.setAleatoriedadPorReiteracionRuta(true);                        
                    }
                    String tipo_dia = conf_dph.getTipoDia();
                    if (tipo_dia.equals("LD") ||
                       (tipo_dia.equals("F")  && dia_festivo) ||
    ...
    public ArrayList<String> recuperarUltimoOrden(Despacho dph, ArrayList<String> lst) {
        ArrayList<String> hlst = hord.get(dph.getIdGrupoMoviles());
    ...
    public void almacenarUltimoOrden(Despacho dph) {
        ArrayList<String> lst = new ArrayList<String>();
        ArrayList<String> l1 = dph.getListaGrupoMoviles();
        ArrayList<String> l2 = dph.getListaGrupoMovilesRetorno();
        ...
        hord.put(dph.getIdGrupoMoviles(), lst);
    }
    ...
    public void inicioGeneracionDespachoRetorno(
        ...
        if (dph.getProgramarRuta() == 1) {
            ...
            if (dph.aleatoriedadPorReiteracionRuta()) {
                lst_mov = recuperarUltimoOrden(dph, lst_mov);
            }
            ...
            // Genera planilla despacho con totalidad de moviles en retorno
            ...
            // Genera planilla despacho con salidas desde punto normal y retorno
            } else {                
                moviles_retorno = true;                
    ...
    public ArrayList<String> obtenerMovilesRetorno(ArrayList<String> lst, int nmov_ret) {
        
        ArrayList<String> nlst = new ArrayList<String>();        
        nmov_ret = (nmov_ret >= lst.size()) ? lst.size()-1 : nmov_ret;
        nmov_ret = (nmov_ret <= 0) ? 0 : nmov_ret;
        
        for (int i = nmov_ret; i >= 1; i--) {
    ...
    public void rotacionAleatoria(Despacho dph, boolean punto_retorno) {
        ...
        int numero_rotacion     = dph.getNumeroRotacion();
        //limite_conservacion = 3;
        
        if (punto_retorno) {
            
            if (numero_rotacion > limite_conservacion) {                
                ArrayList<String> lst_mov     = dph.getListaGrupoMoviles();
                ArrayList<String> lst_mov_ret = dph.getListaGrupoMovilesRetorno();
    ...
    //    public ArrayList<String> rotacionAleatoriaMin(ArrayList<String> lst_mov, 
    //                                                  int limite_conservacion) {
    //        
    //        Permutation p = new Permutation();
    //                
    //        if (ref_nrotaciones >= limite_conservacion) {
    //            ref_nrotaciones = 1;
    //            return p.resetSuccessor(lst_mov);            
    //        } else {
    //            ref_nrotaciones += 1;
    //            return p.nextOrder(lst_mov);
    //        }
    //    }

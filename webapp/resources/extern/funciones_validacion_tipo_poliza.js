...
$(function () {
    $("#btnGuardar").click(function () {
        ...
        if (descripcion == null || descripcion.length == 0) {
            $("#descripcion").focus().after('<span class="error2" id="errorDescripcion">Ingrese una descripción.</span>');  
            $("#descripcion").select();
            return false;
        } else if (descripcion.length > 2000) {
            $("#descripcion").focus().after('<span class="error2" id="errorDescripcion">Límite de 2000 caracteres.</span>');  
            $("#descripcion").select();
            return false;
        }

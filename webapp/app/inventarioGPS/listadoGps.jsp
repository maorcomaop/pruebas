        ...
        <c:choose>
            <c:when test="${permissions.check(65)}">
                <ul class="nav nav-tabs">                    
                    ...
                    <li role="presentation" class="active"><a href="/RDW1/app/inventarioGPS/listadoGps.jsp">Listado documentos</a></li>
                </ul>
                <div class="tab-content">
                    <div role="tabpanel" class="tab-pane padding active" id="pestana1">
                        <table id="tableConductor" class="display" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                <c:choose>
                                    <c:when test="${select.lst_gpsregistrado.size() > 0}">
                                        <tr>                                    
                                            <c:forEach items="${select.lst_gpsregistrado}" var="gps">    
                                                <td>${gps.id}</td>
                                                <td>${gps.marca}</td>
                                                <td>${gps.modelo}</td>
                                                <td>${gps.celular}</td>                                                
                                                <c:choose>
                                                    <c:when test ="${gps.estado == 1}"> 
                                                        <c:if test="${permissions.check(122)}">
                                                            <td>
                                                                <form action="<c:url value='/verGPSInventario' />" method="post">
                                                        ...
                                                        <c:choose>
                                                            <c:when test="${permissions.check(123)}">
                                                                <td>
                                                                    <form action="<c:url value='/eliminarSoat' />" method="post">

        ...
        <c:choose>
            <c:when test="${permissions.check(65)}">
                <ul class="nav nav-tabs">                    
                    ...
                    <li role="presentation" class="active"><a href="/RDW1/app/tarjetaDeOperacion/listadoTarjetaOperacion.jsp">Listado documentos</a></li>
                </ul>
                <div class="tab-content">
                    <div role="tabpanel" class="tab-pane padding active" id="pestana1">
                        <table id="tableConductor" class="display" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                <c:choose>
                                    <c:when test="${select.lstTarjetaOperacion.size() > 0}">
                                        <tr>                                    
                                            <c:forEach items="${select.lstTarjetaOperacion}" var="to">    
                                                <td>${to.codigo}</td>
                                                <td>${to.modelo}</td>
                                                <td>${to.fk_centro_exp}</td>                                                
                                                <td>${to.observaciones}</td>
                                                <td>${to.fecha_vencimiento}</td>
                                                <c:choose>
                                                    <c:when test ="${to.estado == 1}"> 
                                                        <c:if test="${permissions.check(122)}">
                                                            <td>
                                                                <form action="<c:url value='/verTarjetaOperacion' />" method="post">
                                                        ...
                                                        <c:choose>
                                                            <c:when test="${permissions.check(123)}">
                                                                <td>
                                                                    <form action="<c:url value='/eliminarTarjetaOperacion' />" method="post">

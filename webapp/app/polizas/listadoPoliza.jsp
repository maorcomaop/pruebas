<%-- 
    Document   : listadoPoliza
    Created on : 13-jul-2018, 15:31:09
                        ...
                        <table id="tableLicenciaTransito" class="display" cellspacing="0" width="100%">
                            <thead>
                                <tr>
                                    ...
                                    <th>Aseguradora</th>
                                    <th>Tipo Póliza</th>
                                    <th>Amparos Y Coberturas</th>
                                    <c:if test="${permissions.check(191)}">
                                        <th>Editar</th>
                                    </c:if>
                                    <th>Estado</th>
                                </tr>
                            ...
                            <tbody>
                                <c:choose>
                                    <c:when test="${select.lst_polizas.size() > 0}">
                                        <c:forEach items="${select.lst_polizas}" var="poliza">
                                            <tr>
                                                <td>${poliza.nombre}</td>
                                                <td>${poliza.nombreAseguradora}</td>
                                                <td>${poliza.nombreTipoPoliza}</td>
                                                <td>${poliza.amparosCoberturas}</td>
                                                <c:choose>
                                                    <c:when test="${poliza.estado == 1}">
                                                        <c:if test="${permissions.check(191)}">
                                                            <td>
                                                                <form action="<c:url value='/verPoliza' />" method="post">
                                                        ...
                                                        <c:choose>
                                                            <c:when test="${permissions.check(192)}">
                                                                <td>
                                                                    <form action="<c:url value='/cambiarEstadoPoliza' />" method="post" >

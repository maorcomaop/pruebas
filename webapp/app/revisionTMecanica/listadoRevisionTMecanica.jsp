        ...
        <c:choose>
            <c:when test="${permissions.check(65)}">
                <ul class="nav nav-tabs">                    
                    ...
                    <li role="presentation" class="active"><a href="/RDW1/app/revisionTMecanica/listadoRevisionTMecanica.jsp">Listado documentos</a></li>
                </ul>
                <div class="tab-content">
                    <div role="tabpanel" class="tab-pane padding active" id="pestana1">
                        <table id="tableConductor" class="display" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                <c:choose>
                                    <c:when test="${select.lstRevisionMecanica.size() > 0}">
                                        <tr>                                    
                                            <c:forEach items="${select.lstRevisionMecanica}" var="rev">    
                                                <td>${rev.codigo}</td>
                                                <td>${rev.fk_centro_diag}</td>
                                                <td>${rev.fecha_vencimiento}</td>
                                                <td>${rev.observaciones}</td>
                                                <c:choose>
                                                    <c:when test ="${rev.estado == 1}"> 
                                                        <c:if test="${permissions.check(122)}">
                                                            <td>
                                                                <form action="<c:url value='/verTMecanica' />" method="post">
                                                        ...
                                                        <c:choose>
                                                            <c:when test="${permissions.check(123)}">
                                                                <td>
                                                                    <form action="<c:url value='/eliminarTMecanica' />" method="post">

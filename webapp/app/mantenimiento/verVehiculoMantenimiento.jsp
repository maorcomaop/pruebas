<%-- 
    Document   : verVehiculoMantenimiento
    Created on : 03-ago-2018, 10:39:24
            ...
            <c:when test="${permissions.check(202)}">
                <ul class="nav nav-tabs">
                    ...
                    <li role="presentation" class="active">
                        <a href="#">Editar Vehículo En Mantenimiento</a>
                    </li>
                </ul>
                <c:choose>      
                ...
                <div class="tab-content">
                    <div role="tabpanel" class="tab-pane padding active" id="pestana1">
                        ...
                        <form class="form-horizontal formulario" action="<c:url value='/editarVehiculoMantenimiento' />" method="post">
                            <div class="control-group">
                                <div class="control-group">                                        
                                    <div class="controls">
                                        <input type="hidden" name="id_edit" value="${vehiculoMantenimiento.id}">
                                    </div>
                                </div>
                            </div> 
                            <div class="control-group">
                                            ...
                                            <c:forEach items="${select.lst_mantenimiento}" var="mantenimiento">
                                                <c:if test="${mantenimiento.estado eq 1}">
                                                    <option value="${mantenimiento.id}" 
                                                            <c:if test="${vehiculoMantenimiento.fkMantenimiento == mantenimiento.id}">
                                                                selected
                                                            </c:if>>
                                                        ${mantenimiento.nombre}</option>
                                                </c:if>
                                            </c:forEach>
                                        </select>
                                    </div>
                                            ...
                                            <c:forEach items="${select.lstmovil}" var="vehiculo">
                                                <c:if test="${vehiculo.estado eq 1}">
                                                    <option value="${vehiculo.id}"
                                                            <c:if test="${vehiculoMantenimiento.fkVehiculo == vehiculo.id}">
                                                                selected</c:if>>${vehiculo.placa}</option>
                                                </c:if>
                                            </c:forEach>
                                        </select>
                                    </div>
                                ...
                                <div class="row">
                                    <div class="col-sm-6">
                                        <input type="number" name="valorInicialMantenimiento" id="valorInicialMantenimiento" 
                                               ...
                                               min="0" max="9999999" 
                                               value="${vehiculoMantenimiento.valorInicialMantenimiento}">
                                    </div>
                                </div>
                            </div>
                            <div class="control-group">
